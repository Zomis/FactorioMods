-- Functional Library
 --
 -- @file    functional.lua
 -- @author  Shimomura Ikkei, Simon Forsberg
 -- @date    2005/05/18, 2020/05/09
 --
 -- @brief    porting several convenience functional utilities form Haskell,Python etc..

function mapToArray(tbl)
    local index = 1
    local arr = {}
...
function pairsMap(tbl, keys, values)
    local result = {}
    for k, v in pairs(tbl) do
        local new_key = keys(v, k)
        local new_value = values(v, k)
...
function tail(tbl)
    if table_size(tbl) < 1 then
        return nil
    else
        local newtbl = {}
        local tblsize = table_size(tbl)
        local i, v = next(tbl, next(tbl, nil))
        while i ~= nil do
            newtbl[i] = v
            i, v = next(tbl, i)
        end
...
enumFromTo = function (from,to)
    ...
    local val = from
    while val <= to do
        table.insert(newtbl, table_size(newtbl)+1, val)
        val = step(val)
    end
    return newtbl
end
