...
local function open(player)
    player.cursor_stack.set_stack({ name = "foogle" })
...
local function on_dropped_item(event)
    ...
    if event.entity.stack.name == "foogle" then
        event.entity.stack.clear()
    end
end

local function clear_foogle_selector(player)
    local stack = player.cursor_stack
    if stack.valid_for_read and stack.name == "foogle" then
        stack.clear()
    end
end

local function on_player_selected_area(event)
    if event.item ~= "foogle" then
        return
    end
    local player = game.players[event.player_index]
    clear_foogle_selector(player)
